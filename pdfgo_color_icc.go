...
func (r *Reader) readICCRGB(object Array) (*iccRGBSpace, error) {
	...
	if stream.Dictionary["N"] != Integer(3) {
		return nil, &UnsupportedError{Feature: "non-RGB ICC profile"}
	}
...
func parseICCRGB(data []byte) (*iccRGBSpace, error) {
	...
	if string(data[16:20]) != "RGB " || string(data[20:24]) != "XYZ " || data[8] != 2 && data[8] != 4 {
	...
	for i, channel := range []string{"r", "g", "b"} {
		...
		curve := tags[channel+"TRC"]
		if len(curve) < 12 || string(curve[:4]) != "curv" {
			return nil, &UnsupportedError{Feature: "ICC tone curve type"}
		}
		n := uint64(binary.BigEndian.Uint32(curve[8:]))
		if n > uint64(len(curve)-12)/2 {
			return nil, fmt.Errorf("invalid ICC tone curve length")
		}
		s.curves[i] = make([]uint16, int(n))
		for j := range s.curves[i] {
			s.curves[i][j] = binary.BigEndian.Uint16(curve[12+2*j:])
			if j > 0 && s.curves[i][j] < s.curves[i][j-1] {
				return nil, fmt.Errorf("nonmonotonic ICC tone curve")
			}
		}
		if n == 1 && s.curves[i][0] == 0 {
